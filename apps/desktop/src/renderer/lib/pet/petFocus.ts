...
import { mapDashboardStatus, type AgentDashboardStatus } from "./pixLiveModel";
import type { SessionLiveMap, SessionLiveSnapshot } from "./pixLiveModel";
...
export function kindForSession(sessionId: string, input: PetFocusInput): PetKind {
...
export function pickPetFocus(input: PetFocusInput): PetFocus {
  ...
  for (const id of ids) {
    ...
    if (
      rank < bestRank ||
      (rank === bestRank &&
        (activity > bestActivity || (activity === bestActivity && id < (bestId ?? ""))))
    ) {
      bestId = id;
      bestKind = kind;
...
export function resolvePetFocus(
  ...
  if (prevId && kindForSession(prevId, input) === winningKind) {
  ...
  for (const id of ids) {
    ...
    if (activity > bestActivity || (activity === bestActivity && id < (bestId ?? ""))) {
      bestId = id;
      bestActivity = activity;
    }
  }
...
export function petVerbFor(kind: PetKind, toolTitle?: string | null): PetVerb {
  switch (kind) {
    case "needs_you":
      return "waiting";
    case "error":
    ...
    case "working": {
      ...
      if (/\b(write|edit|apply_patch|str_replace|strreplace|create_file)\b/.test(t)) {
        return "writing";
      }
      return "working";
    }
