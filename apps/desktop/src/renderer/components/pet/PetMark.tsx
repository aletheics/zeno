...
export function PetMark({
  ...
  const fill = resolvePetBodyInk(isPetColor(color) ? color : "black");
  const eyeInk = resolvePetEyeInk(isPetColor(color) ? color : "black", eyeColor);
  ...
  applyFrameRef.current = (frame: BotFrame) => {
    ...
    for (let i = 0; i < ARC_SLOTS; i++) {
      ...
      if (grad) {
        ...
        for (let j = 0; j < 3; j++) {
          setAttrStr(
            (pool?.[j + 1] ?? null) as CachedEl | null,
            "stop-color",
            arc.grad.stops[j] ?? "#fff",
          );
        }
      }
    ...
    if (dotsBehindRef.current !== frame.dotsBehind) {
      ...
      const idle = frame.dotsBehind
        ? [dotFrontCircles.current, dotFrontPaths.current]
        : [dotBackCircles.current, dotBackPaths.current];
  ...
  useEffect(() => {
    ...
    if (!hadEngine) {
      ...
      applyFrameRef.current(engine.sample(pausedRef.current ? (POSES[play0.state] ?? 1) : 0));
    }
    ...
    if (!pausedRef.current && !reduce && !restOnlyRef.current) {
      pullOverlayFrame();
      void (async () => {
        unlistenMoved = await onWindowMoved(() => {
          pullOverlayFrame();
        });
      })();
    ...
    const resolvePlay = (nowMs: number) => {
      ...
      const trackingLook =
        nearMark && session !== "sleeping" && session !== "dragging" && !draggingRef.current;
    ...
    const paint = (clock: number, dt: number) => {
      ...
      syncFaceMirror();
      const baseFace = shownState === "idle" || shownState === "swirl";
      ...
      if (!axes) {
        ...
        if (Number.isFinite(nx) && Number.isFinite(ny)) {
          const enter = !aiming;
          if (enter || Math.abs(nx - lastLookNx) > 0.02 || Math.abs(ny - lastLookNy) > 0.02) {
            engine.setLook(
    ...
    const tick = (ms: number) => {
      ...
      const morphing =
        wantSpinRef.current !== playedSpinRef.current || engine.isMorphing(clockRef.current);
      const catalogLive = bloubStateNeedsLivePaint(engine.state);
      const beltsLive = orbit.hasLife();
      if (trackingLook || spin || morphing || draggingRef.current || catalogLive || beltsLive) {
        idleSince = nowMs;
      }
      const minMs = petPaintMinMs({
        spinning: spin != null || wantSpinRef.current !== playedSpinRef.current || beltsLive,
        morphing,
      ...
      <defs>
        ...
        {Array.from({ length: ARC_SLOTS }, (_, i) => (
          <linearGradient
            key={i}
            ref={(el) => {
              gradRefs.current[i] = [el, ...(gradRefs.current[i]?.slice(1) ?? [])];
            }}
            id={`${uid}-arc${i}`}
