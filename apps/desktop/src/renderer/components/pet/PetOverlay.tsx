 ...
 * Pet settings opens the same Settings → 宠物 section as the settings nav item.
 */
...
import { onWindowMoved } from "@/lib/api/host";
...
export function PetOverlay({
  ...
  const color = isPetColor(prefs.color) ? prefs.color : "black";
  ...
  const spinWatchRef = useRef<{
    ...
    doneIds: string[];
  }>({ primed: false, kind: null, doneIds: [] });
  ...
  const hugMark = policy.compactIdle && !menuOpen && !(bubblesOn && tasks.length > 0);
  ...
  const title = [statusLabel, focus.title, focus.toolTitle].filter(Boolean).join(" · ");
  ...
  useEffect(() => {
    ...
    const nextDoneIds = petDoneTaskIds(tasks);
    if (
      shouldTriggerPetSpin({
        primed: prev.primed,
        prevKind: prev.kind,
        nextKind: focus.kind,
        prevDoneIds: new Set(prev.doneIds),
        nextDoneIds: new Set(nextDoneIds),
      })
    ) {
      setSpinSignal((n) => n + 1);
    }
    spinWatchRef.current = {
      primed: true,
      kind: focus.kind,
      doneIds: nextDoneIds,
    };
    ...
    >
      ...
      <PetContextMenu
        ...
        items={[
          ...
          {
            id: "pet-bubbles",
            label: bubblesOn ? t("pet.menu.hideBubbles") : t("pet.menu.showBubbles"),
            onClick: () => {
